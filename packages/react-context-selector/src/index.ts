...
function createProvider<T>(ProviderOriginal: Provider<OverridenContextValue<T>>) {
    ...
    const Provider = ({ value, children }: { value: any; children: ReactNode; }) => {
        ...
        const values = useMemo(() => ({
            subscribe: (listener: Listener) => {
                ...
                return () => {
                    listeners.current = listeners.current.filter(l => listener !== l);
                }
            },
...
function createContext<TOriginalValue>(initValues: TOriginalValue) {
    const context = createContextOriginal<OverridenContextValue<TOriginalValue>>({ getValues: () => initValues, subscribe: () => () => { } });

    (context as unknown as Context<OverridenContextValue<TOriginalValue>>).Provider = createProvider<TOriginalValue>(context.Provider);
...
function useContextSelector<TOriginalValue, TSelectedValue>(
    ...
    // TODO rtr look to do better?
    const { subscribe, getValues } = useContextOriginal(context as ContextOriginal<OverridenContextValue<TOriginalValue>>);

    return useSyncExternalStore<TSelectedValue>(subscribe, () => selector(getValues()));
}
